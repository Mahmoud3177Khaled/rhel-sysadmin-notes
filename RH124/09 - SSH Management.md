...
**"/etc/ssh/ssh_host_*"** ---> Are my keys i send to people trying to ssh into me to prove to 																		  them i am who they think i am.
																  They are automatically generated by sshd, if i wanna change them:
															   **rm -f "/etc/ssh/ssh_host_*"**, then **systemctl restart sshd**
...
**ssh-keyscan 172.25.250.0/24 >> /etc/ssh/ssh_known_hosts** → Scan entire subnet for 																																									 		  keys. and save them where users 																																					  look if they didnt recognize a 																																							  server using their known_hosts...
